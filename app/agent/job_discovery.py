    ...
    async def analyze_job_market(self, query: str = None) -> Dict:
        ...
        try:
            ...
            for job in recent_jobs:
                ...
                analysis["job_types"][job_type] = analysis["job_types"].get(job_type, 0) + 1
            
            # Count remote types
            for job in recent_jobs:
                remote_type = job.remote_type.value if job.remote_type else "Not specified"
                analysis["remote_types"][remote_type] = analysis["remote_types"].get(remote_type, 0) + 1
            ...
            for job in recent_jobs:
                if job.skills_required:
                    for skill in job.skills_required:
                        analysis["top_skills"][skill] = analysis["top_skills"].get(skill, 0) + 1
            
            # Count companies
            for job in recent_jobs:
                analysis["top_companies"][job.company] = analysis["top_companies"].get(job.company, 0) + 1
            
            # Sort and limit top items
            analysis["locations"] = dict(sorted(analysis["locations"].items(), key=lambda x: x[1], reverse=True)[:10])
            analysis["top_skills"] = dict(sorted(analysis["top_skills"].items(), key=lambda x: x[1], reverse=True)[:10])
            analysis["top_companies"] = dict(sorted(analysis["top_companies"].items(), key=lambda x: x[1], reverse=True)[:10])
            
            logger.info(f"Job market analysis completed: {len(recent_jobs)} jobs analyzed")
            return analysis
