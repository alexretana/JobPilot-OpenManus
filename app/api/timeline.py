...
from app.data.models import TimelineEvent, TimelineEventType
...
def get_user_timeline(
    ...
    days_back: Optional[int] = Query(None, ge=1, description="Only include events from the last N days"),
...
def get_job_timeline(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error retrieving job timeline: {str(e)}")
...
def get_application_timeline(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error retrieving application timeline: {str(e)}")


@router.get("/user/{user_profile_id}/milestones", response_model=List[TimelineEventResponse])
def get_user_milestones(
    user_profile_id: str,
    limit: int = Query(20, ge=1, le=100, description="Maximum number of milestones to return"),
    days_back: Optional[int] = Query(30, ge=1, description="Only include milestones from the last N days"),
    db: Session = Depends(get_database_session)
):
...
@router.get("/user/{user_profile_id}/upcoming", response_model=List[TimelineEventResponse])
...
def create_timeline_event(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error creating timeline event: {str(e)}")


@router.post("/user/{user_profile_id}/custom-event", response_model=TimelineEventResponse)
def create_custom_event(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error creating custom event: {str(e)}")
...
@router.post("/user/{user_profile_id}/job/{job_id}/saved", response_model=TimelineEventResponse)
def log_job_saved(
    user_profile_id: str,
    job_id: str,
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error logging job saved event: {str(e)}")


@router.post("/user/{user_profile_id}/application/{application_id}/submitted", response_model=TimelineEventResponse)
def log_application_submitted(
    ...
    job_title: str = Query(..., description="Job title"),
    company_name: str = Query(..., description="Company name"),
    application_method: Optional[str] = Query(None, description="How the application was submitted"),
    db: Session = Depends(get_database_session)
):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error logging application submitted event: {str(e)}")


@router.post("/user/{user_profile_id}/application/{application_id}/interview-scheduled", response_model=TimelineEventResponse)
def log_interview_scheduled(
    ...
    interview_type: Optional[str] = Query(None, description="Type of interview (phone, video, in-person, etc.)"),
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error logging interview scheduled event: {str(e)}")


@router.post("/user/{user_profile_id}/application/{application_id}/status-changed", response_model=TimelineEventResponse)
def log_status_change(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error logging status change event: {str(e)}")
