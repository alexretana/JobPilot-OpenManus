...
def get_leads(
    ...
    try:
        ...
        user_leads = [lead for lead in leads_storage if lead.user_profile_id == user_profile_id]
...
def get_lead_stats(user_profile_id: str = Query(DEFAULT_USER_ID, description="User profile ID")):
    ...
    try:
        ...
        for status in LeadStatus:
            status_counts[status.value] = len([lead for lead in user_leads if lead.status == status])
        
        # Type breakdown
        type_counts = {}
        for lead_type in LeadType:
            type_counts[lead_type.value] = len([lead for lead in user_leads if lead.lead_type == lead_type])
