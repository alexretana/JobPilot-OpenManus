...
from app.data.resume_models import (
    Resume, ResumeDB, ResumeTemplateDB, SkillBankDB, ResumeGenerationDB,
    ResumeOptimizationDB, ATSScore, ContactInfo, WorkExperience, Education,
    Skill, Project, Certification, ResumeStatus, ResumeType, SectionType,
    create_resume_from_profile, calculate_resume_completeness, generate_ats_score
)
from app.data.models import UserProfileDB, JobListingDB
...
class ResumeRepository:
    ...
    async def create_resume(self, resume_data: Resume) -> ResumeDB:
        ...
        try:
            ...
            serialized_work_experience = self._serialize_dates_in_dict(resume_dict.get('work_experience', []))
            serialized_education = self._serialize_dates_in_dict(resume_dict.get('education', []))
            serialized_skills = self._serialize_dates_in_dict(resume_dict.get('skills', []))
            serialized_projects = self._serialize_dates_in_dict(resume_dict.get('projects', []))
            serialized_certifications = self._serialize_dates_in_dict(resume_dict.get('certifications', []))
            serialized_custom_sections = self._serialize_dates_in_dict(resume_dict.get('custom_sections', []))
    ...
    async def get_resume(self, resume_id: str, user_id: str) -> Optional[Resume]:
        ...
        try:
            resume_db = self.session.query(ResumeDB).filter(
                and_(ResumeDB.id == resume_id, ResumeDB.user_id == user_id)
            ).first()
    ...
    async def update_resume(self, resume_id: str, user_id: str, updates: Dict[str, Any]) -> Optional[Resume]:
    ...
    async def create_resume_from_user_profile(self, user_id: str, title: str = None) -> Optional[Resume]:
        """Create a basic resume from user profile data."""
        try:
            # Get user profile
            user_profile = self.session.query(UserProfileDB).filter(UserProfileDB.id == user_id).first()
            if not user_profile:
    ...
    async def create_tailored_resume(self, base_resume_id: str, job_id: str, user_id: str) -> Optional[Resume]:
        ...
        try:
            ...
            # Get job details
            job_db = self.session.query(JobListingDB).filter(JobListingDB.id == job_id).first()
            if not job_db:
    ...
    async def optimize_resume_for_job(self, resume_id: str, job_id: str, user_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            # Calculate ATS score
            job_description = f"{job_db.title} {job_db.description or ''} {job_db.requirements or ''}"
            ...
            optimization_db = ResumeOptimizationDB(
                ...
                sections_to_emphasize=[SectionType.SKILLS.value, SectionType.EXPERIENCE.value],
                content_suggestions=[
    ...
    async def get_resume_templates(self) -> List[Dict[str, Any]]:
        ...
        try:
            templates = self.session.query(ResumeTemplateDB).filter(
                or_(ResumeTemplateDB.is_system == True, ResumeTemplateDB.is_default == True)
            ).all()
        ...
        try:
            default_templates = [
                ...
                },
                {
                    ...
                    "sections": ["contact", "summary", "projects", "experience", "skills", "education"],
            ...
            for template_data in default_templates:
                existing = self.session.query(ResumeTemplateDB).filter(
                    ResumeTemplateDB.name == template_data["name"]
                ).first()
    ...
    async def get_user_skill_bank(self, user_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            skill_bank = self.session.query(SkillBankDB).filter(
                SkillBankDB.user_id == user_id
            ).first()
    ...
    async def update_skill_bank_from_resume(self, user_id: str, resume: Resume):
        ...
        try:
            ...
            for skill in resume_skills:
                confidence[skill] = confidence.get(skill, 0.5) + 0.1  # Increment confidence
    ...
    def _serialize_dates_in_dict(self, data: Any) -> Any:
        ...
        if isinstance(data, dict):
            return {key: self._serialize_dates_in_dict(value) for key, value in data.items()}
    ...
    def _deserialize_dates_in_dict(self, data: Any) -> Any:
        ...
        if isinstance(data, dict):
            ...
            for key, value in data.items():
                if key in ['start_date', 'end_date', 'issue_date', 'expiry_date'] and isinstance(value, str):
                    try:
