...
class ETLEnhancedSettings(BaseSettings):
    ...
    jsearch_rate_limit_per_minute: int = Field(default=60, env="JSEARCH_RATE_LIMIT_PER_MINUTE")
    jsearch_max_pages_per_query: int = Field(default=5, env="JSEARCH_MAX_PAGES_PER_QUERY")
    ...
    processing_enable_embeddings: bool = Field(default=True, env="PROCESSING_ENABLE_EMBEDDINGS")
    processing_enable_deduplication: bool = Field(default=True, env="PROCESSING_ENABLE_DEDUPLICATION")
    processing_quality_threshold: float = Field(default=0.7, env="PROCESSING_QUALITY_THRESHOLD")
    ...
    duplicate_location_weight: float = Field(default=0.2, env="DUPLICATE_LOCATION_WEIGHT")
    ...
    # Alert settings
    alert_failure_rate_threshold: float = Field(default=0.2, env="ALERT_FAILURE_RATE_THRESHOLD")
    alert_response_time_threshold: int = Field(default=300, env="ALERT_RESPONSE_TIME_THRESHOLD")
    ...
    scheduler_misfire_grace_time: int = Field(default=300, env="SCHEDULER_MISFIRE_GRACE_TIME")
    ...
    enable_job_prioritization: bool = Field(default=True, env="ENABLE_JOB_PRIORITIZATION")
    ...
    @validator("data_directory", "logs_directory", "config_directory", "backup_directory")
    ...
    def validate_settings(self) -> List[str]:
        ...
        for dir_path in [self.data_directory, self.logs_directory, self.backup_directory]:
...
class ETLSettingsManager:
    ...
    def load_settings(self, env_file: Optional[str] = None) -> ETLEnhancedSettings:
        ...
        try:
            ...
            logger.info(f"Loaded ETL settings for environment: {self._settings.environment}")
            return self._settings
        ...
        if jobs_config_file.exists():
            try:
                ...
                if jobs_config and "jobs" in jobs_config:
                    ...
                    logger.info(f"Loaded {len(jobs_config['jobs'])} custom job configurations")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        if command == "create-samples":
            ...
            output_file = Path(sys.argv[2] if len(sys.argv) > 2 else "config_export.yaml")
            manager = get_settings_manager()
            manager.export_config(output_file)
            print(f"✅ Configuration exported to {output_file}")
            
        else:
            print("Usage: python settings.py [create-samples|validate|export [output_file]]")
            sys.exit(1)
    else:
