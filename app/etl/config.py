...
class ETLConfig:
    ...
    api_rate_limit_per_minute: int = field(
        default_factory=lambda: int(os.getenv("API_RATE_LIMIT_PER_MINUTE", "10"))
    )
    api_timeout_seconds: int = 30

    # JSearch API Configuration
    jsearch_base_url: str = "https://jsearch.p.rapidapi.com"
    jsearch_endpoints: Dict[str, str] = field(
    ...
    database_url: str = field(
        default_factory=lambda: os.getenv("DATABASE_URL", "sqlite:///data/jobpilot.db")
    )

    # File Storage Configuration
    raw_data_dir: Path = field(default_factory=lambda: Path("data/raw_collections"))
    processed_data_dir: Path = field(
        default_factory=lambda: Path("data/processed_data")
    )
    failed_data_dir: Path = field(
        default_factory=lambda: Path("data/failed_processing")
    )
    ...
    schedule_enabled: bool = field(
        default_factory=lambda: os.getenv("ETL_SCHEDULE_ENABLED", "false").lower()
        == "true"
    )
