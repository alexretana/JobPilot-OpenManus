...
from ..data.models import (
    # SQLAlchemy models
    JobListingDB, JobSourceDB, JobSourceListingDB, JobEmbeddingDB, 
    JobDeduplicationDB, ProcessedJobDataDB, ETLOperationLogDB,
    
    # Pydantic models
    JobListing, JobSource, JobSourceListing, JobEmbedding, JobDeduplication,
    ProcessedJobData, ETLOperationLog,
    
    # Enums
    ETLProcessingStatus, ETLOperationType, VerificationStatus, JobStatus,
...
class DuplicationDetector:
    ...
    def find_duplicates(self, job_data: Dict[str, Any], session: Session) -> Optional[str]:
        """
        ...
        if job_url := job_data.get("job_url"):
            existing = session.query(JobListingDB).filter(
    ...
    def _find_by_title_company_location(self, job_data: Dict[str, Any], session: Session) -> Optional[str]:
        ...
        candidates = session.query(JobListingDB).filter(
            and_(
                JobListingDB.title.ilike(f"%{title}%"),
                JobListingDB.company.ilike(f"%{company}%"),
                JobListingDB.status == JobStatus.ACTIVE
            )
        ).limit(10).all()  # Limit to avoid performance issues
        
        for candidate in candidates:
            if self._calculate_similarity_score(job_data, candidate) > self.similarity_threshold:
                logger.debug(f"Found similar job match: {candidate.id}")
                return candidate.id
        
        return None
    
    def _calculate_similarity_score(self, job_data: Dict[str, Any], existing_job: JobListingDB) -> float:
...
class JobDataLoader:
    ...
    async def load_processed_batch(self, processing_id: str) -> str:
        ...
        # Start operation log
        operation_log = await self._start_operation_log("batch_load", {"processing_id": processing_id})
        ...
        try:
            ...
            if not processed_jobs:
                logger.warning(f"No processed jobs found for processing ID {processing_id}")
            ...
            status = ETLProcessingStatus.COMPLETED if self.batch_stats["errors"] == 0 else ETLProcessingStatus.PARTIAL
            await self._complete_operation_log(operation_log.id, status, dict(self.batch_stats))
    ...
    async def _load_single_job(self, processed_job: ProcessedJobData):
        ...
        with self.db_manager.get_session() as session:
            ...
            if duplicate_job_id:
                ...
                await self._handle_duplicate_job(processed_job, duplicate_job_id, session)
    ...
    async def _create_new_job(self, processed_job: ProcessedJobData, session: Session) -> str:
    ...
    async def _handle_duplicate_job(self, processed_job: ProcessedJobData, duplicate_job_id: str, session: Session):
        """Handle a duplicate job by updating the existing one or recording the duplicate relationship."""
        # Get the existing job
        existing_job = session.query(JobListingDB).filter(
            JobListingDB.id == duplicate_job_id
        ).first()
        
        if not existing_job:
            logger.warning(f"Duplicate job {duplicate_job_id} not found, creating new job")
        ...
        logger.debug(f"Handled duplicate job: canonical={duplicate_job_id}, duplicate={new_job_db.id}")
    
    async def _store_job_embeddings(self, job_id: str, processed_job: ProcessedJobData, session: Session):
        """Store job embeddings in the database."""
        if not processed_job.embedding_vector:
            return
        
        # Create content hash
        content = processed_job.processed_data.get("description", "") + " " + processed_job.processed_data.get("title", "")
    ...
    async def _get_processed_jobs(self, processing_id: str) -> List[ProcessedJobData]:
        ...
        try:
            with self.db_manager.get_session() as session:
                ...
                ).order_by(ProcessedJobDataDB.job_index).all()
    ...
    async def _update_processed_job_status(self, processing_id: str, job_index: int, status: ETLProcessingStatus):
        """Update the load status of a processed job."""
        try:
            with self.db_manager.get_session() as session:
                job_db = session.query(ProcessedJobDataDB).filter(
                    and_(
                        ProcessedJobDataDB.processing_id == processing_id,
    ...
    async def cleanup_old_duplicates(self, days_old: int = 30) -> int:
        ...
        try:
            with self.db_manager.get_session() as session:
                old_duplicates = session.query(JobDeduplicationDB).filter(
                    and_(
                        JobDeduplicationDB.reviewed == False,
                        JobDeduplicationDB.created_at < cleanup_date,
                        JobDeduplicationDB.confidence_score < 0.7  # Only low confidence ones
                    )
                ).all()
    ...
    async def get_load_statistics(self, start_date: datetime = None) -> Dict[str, Any]:
        ...
        try:
            with self.db_manager.get_session() as session:
                ...
                total_jobs = session.query(JobListingDB).filter(
                    JobListingDB.created_at >= start_date
                ).count()
                
                # Count duplicates found
                total_duplicates = session.query(JobDeduplicationDB).filter(
                    JobDeduplicationDB.created_at >= start_date
                ).count()
                
                # Count embeddings generated
                total_embeddings = session.query(JobEmbeddingDB).filter(
                    JobEmbeddingDB.created_at >= start_date
                ).count()
                
                # Get recent operations
                recent_ops = session.query(ETLOperationLogDB).filter(
                    and_(
                        ETLOperationLogDB.operation_type == ETLOperationType.LOADING,
...
async def load_all_pending_batches(loader: JobDataLoader) -> Dict[str, Any]:
    ...
    try:
        with loader.db_manager.get_session() as session:
            ...
            pending_batches = session.query(ProcessedJobDataDB.processing_id).filter(
                ProcessedJobDataDB.load_status == ETLProcessingStatus.PENDING
            ).distinct().all()
