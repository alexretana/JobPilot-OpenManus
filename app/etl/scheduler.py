...
from ..data.models import (
    ETLOperationLog, ETLProcessingStatus, ETLOperationType,
    RawJobCollection, JobProcessingLog, ProcessedJobData,
    pydantic_to_sqlalchemy, ETLOperationLogDB
)
...
class ETLOrchestrator:
    ...
    async def run_full_pipeline(self, 
                               collection_params: Dict[str, Any] = None,
                               max_concurrent_jobs: int = 3) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            collection_result = await self._run_collection_phase(collection_params or {})
            ...
            logger.info(f"ETL pipeline completed: {results['overall_status']} in {results['total_duration_seconds']}s")
        ...
        try:
            ...
            async def process_collection(collection: RawJobCollection):
                async with semaphore:
                    try:
                        processing_id = await self.processor.process_collection(collection.id)
                        return {"collection_id": collection.id, "processing_id": processing_id, "status": "success"}
                    except Exception as e:
                        logger.error(f"Error processing collection {collection.id}: {e}")
                        return {"collection_id": collection.id, "error": str(e), "status": "error"}
            
            # Execute processing tasks
            processing_tasks = [process_collection(collection) for collection in pending_collections]
            ...
            for processing_result in processing_results:
                if isinstance(processing_result, Exception):
                    ...
                    failed_processing += 1
                    result["errors"].append(processing_result.get("error", "Unknown processing error"))
    ...
    async def _run_loading_phase(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if loading_results.get("fatal_error"):
                ...
                result["status"] = ETLProcessingStatus.PARTIAL
                result["errors"] = [br.get("error") for br in loading_results["batch_results"] if br.get("status") == "error"]
            else:
                result["status"] = ETLProcessingStatus.COMPLETED
    ...
    async def run_maintenance_tasks(self) -> Dict[str, Any]:
        ...
        try:
            ...
            result["duration_seconds"] = (maintenance_end - maintenance_start).total_seconds()
    ...
    async def _assess_pipeline_health(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if recent_operations:
                failed_ops = [op for op in recent_operations if op["status"] == ETLProcessingStatus.FAILED]
                failure_rate = len(failed_ops) / len(recent_operations)
                
                if failure_rate > 0.2:  # More than 20% failure rate
                    health["status"] = "unhealthy"
                    health["issues"].append(f"High failure rate: {failure_rate:.1%}")
                    health["recommendations"].append("Investigate recurring failures and improve error handling")
                
                elif failure_rate > 0.1:  # More than 10% failure rate
                    health["status"] = "warning"
                    health["issues"].append(f"Elevated failure rate: {failure_rate:.1%}")
                    health["recommendations"].append("Monitor for patterns in failures")
            
            # Check for stale data
            with self.db_manager.get_session() as session:
                from ..data.models import RawJobCollectionDB
                latest_collection = session.query(RawJobCollectionDB).order_by(
                    RawJobCollectionDB.created_at.desc()
                ).first()
                
                if latest_collection:
                    hours_since_collection = (datetime.utcnow() - latest_collection.created_at).total_seconds() / 3600
                    if hours_since_collection > 48:  # No collection in 48 hours
                        health["status"] = "warning"
                        health["issues"].append(f"No data collection in {hours_since_collection:.1f} hours")
                        health["recommendations"].append("Check collection schedule and API connectivity")
        ...
        try:
            with self.db_manager.get_session() as session:
                ...
                recent_ops = session.query(ETLOperationLogDB).filter(
                    ETLOperationLogDB.started_at >= cutoff_time
                ).order_by(ETLOperationLogDB.started_at.desc()).all()
...
class ETLScheduler:
    ...
    def _add_scheduled_job(self, job_config: ETLJobConfig):
        ...
        try:
            ...
            logger.info(f"Scheduled job '{job_config.name}' with schedule '{job_config.schedule}'")
    ...
    async def _run_collection_job(self, job_config: ETLJobConfig, parameters: Dict[str, Any]):
        ...
        try:
            ...
            logger.info(f"Collection job {job_config.name} completed: {result['status']}")
            return result
    ...
    async def _run_processing_job(self, job_config: ETLJobConfig, parameters: Dict[str, Any]):
        ...
        try:
            ...
            logger.info(f"Processing job {job_config.name} completed: {result['status']}")
            return result
    ...
    async def _run_loading_job(self, job_config: ETLJobConfig, parameters: Dict[str, Any]):
    ...
    async def _run_maintenance_job(self, job_config: ETLJobConfig, parameters: Dict[str, Any]):
        ...
        try:
            ...
            logger.info(f"Maintenance job {job_config.name} completed: {len(result.get('tasks', {}))} tasks")
            return result
    ...
    async def _run_full_pipeline_job(self, job_config: ETLJobConfig, parameters: Dict[str, Any]):
        ...
        try:
            ...
            logger.info(f"Full pipeline job {job_config.name} completed: {result['overall_status']}")
            return result
    ...
    def get_job_status(self) -> Dict[str, Any]:
        ...
        for job in self.scheduler.get_jobs():
            job_info = {
                ...
                "pending": len([t for t in self.orchestrator.active_jobs.values() if t.get_name() == job.id])
            }
