...
class SemanticSearchTool(BaseTool):
    ...
    async def _run(
        ...
        try:
            ...
            job_type_list = [jt.strip() for jt in job_types.split(",") if jt.strip()] if job_types else None
            remote_type_list = [rt.strip() for rt in remote_types.split(",") if rt.strip()] if remote_types else None  
            ...
            if embedding_service:
                ...
                matches = await self._semantic_search_with_embeddings(
                    query, parsed_job_types, parsed_remote_types, location_list,
                    min_salary or None, max_salary or None, max_results
                )
            else:
