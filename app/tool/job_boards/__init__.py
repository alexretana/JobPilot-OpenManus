...
from .base_scraper import JobBoardScraper, JobSearchQuery, RawJobData, RateLimiter
...
def get_scraper(name: str) -> JobBoardScraper:
    ...
    if name not in AVAILABLE_SCRAPERS:
        raise ValueError(f"Unknown scraper: {name}. Available: {list(AVAILABLE_SCRAPERS.keys())}")
