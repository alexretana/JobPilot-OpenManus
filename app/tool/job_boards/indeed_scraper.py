...
from .base_scraper import JobBoardScraper, JobSearchQuery, RawJobData, clean_text, extract_salary_info
...
class IndeedScraper(JobBoardScraper):
    ...
    async def search_jobs(self, query: JobSearchQuery) -> List[RawJobData]:
        ...
        try:
            ...
            if not job_cards:
                ...
                if job_cards:
                    logger.info(f"Found {len(job_cards)} job cards using alternative selector")
                else:
    ...
    async def parse_job_data(self, raw_data: Dict[str, Any], source_url: str) -> RawJobData:
    ...
    async def _parse_job_card(self, card, base_url: str) -> Optional[RawJobData]:
        ...
        try:
            ...
            job_link = card.find('a', {'data-jk': True}) or card.find('h2', {'class': re.compile(r'jobTitle')}).find('a')
            if not job_link:
                return None
            
            job_id = job_link.get('data-jk') or job_link.get('href', '').split('jk=')[-1].split('&')[0]
            ...
            company_elem = card.find('span', {'class': re.compile(r'companyName')}) or card.find('a', {'data-tn-element': 'companyName'})
            company = clean_text(company_elem.get_text()) if company_elem else 'Unknown Company'
            
            location_elem = card.find('div', {'class': re.compile(r'companyLocation')}) or card.find('span', {'class': re.compile(r'location')})
            location = clean_text(location_elem.get_text()) if location_elem else 'Unknown Location'
            ...
            snippet_elem = card.find('div', {'class': re.compile(r'job-snippet|summary')})
    ...
    async def _parse_job_detail_page(self, soup: BeautifulSoup, job_url: str) -> Optional[RawJobData]:
        ...
        try:
            ...
            company_elem = soup.find('div', {'class': re.compile(r'companyName')}) or soup.find('a', {'data-tn-element': 'companyName'})
            company = clean_text(company_elem.get_text()) if company_elem else 'Unknown Company'
            ...
            desc_elem = soup.find('div', {'class': re.compile(r'jobDescriptionText|jobsearch-jobDescriptionText')})
            description = clean_text(desc_elem.get_text()) if desc_elem else ''
            
            # Salary information
            salary_elem = soup.find('span', {'class': re.compile(r'salary|icl-u-xs-mr--xs')})
            salary_info = clean_text(salary_elem.get_text()) if salary_elem else None
            
            # Job type (full-time, part-time, etc.)
            job_type_elem = soup.find('div', {'class': re.compile(r'jobType|attribute')})
            ...
            company_url = urljoin(self.base_url, company_link.get('href', '')) if company_link else None
