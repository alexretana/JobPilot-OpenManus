...
from app.data.models import (
    TimelineEvent, TimelineEventDB, TimelineEventType,
    JobApplication, JobApplicationDB, SavedJob, SavedJobDB,
    JobListing, JobListingDB, UserProfile, UserProfileDB,
    sqlalchemy_to_pydantic, pydantic_to_sqlalchemy
)
...
class TimelineService:
    ...
    def get_user_timeline(
        ...
        try:
            ...
            return [sqlalchemy_to_pydantic(event_db, TimelineEvent) for event_db in events_db]
    ...
    def get_job_timeline(
        ...
        try:
            ...
            # Order by event date (most recent first)
            events_db = query.order_by(desc(TimelineEventDB.event_date)).limit(limit).all()
    ...
    def update_event(
        ...
        try:
            event_db = self.db.query(TimelineEventDB).filter(
                TimelineEventDB.id == event_id
            ).first()
    ...
    def get_upcoming_events(
        ...
        try:
            ...
            events_db = self.db.query(TimelineEventDB).filter(
                and_(
                    TimelineEventDB.user_profile_id == user_profile_id,
                    TimelineEventDB.event_date > datetime.utcnow(),
                    TimelineEventDB.event_date <= future_date
                )
            ).order_by(TimelineEventDB.event_date).limit(limit).all()
