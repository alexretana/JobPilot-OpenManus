...
try:
    import rendercv
    from rendercv.cli import commands
    RENDERCV_AVAILABLE = True
except ImportError:
    RENDERCV_AVAILABLE = False

from app.data.resume_models import Resume, WorkExperience, Education, Skill, Project, Certification
...
class PDFGenerationService:
    ...
    def __init__(self, output_dir: Optional[str] = None):
        ...
        self.output_dir = Path(output_dir) if output_dir else Path.cwd() / "generated_resumes"
        ...
        try:
            ...
            with tempfile.TemporaryDirectory() as temp_dir:
                ...
                with open(yaml_file, 'w', encoding='utf-8') as f:
                    yaml.dump(rendercv_data, f, default_flow_style=False, allow_unicode=True)
                
                # Generate output filename
                if not output_filename:
                    safe_name = "".join(c for c in resume.contact_info.full_name if c.isalnum() or c in (' ', '-', '_')).rstrip()
    ...
    def _convert_to_rendercv_format(self, resume: Resume) -> Dict[str, Any]:
        ...
        if resume.contact_info.linkedin:
            social_networks.append({
                "network": "LinkedIn",
                "username": self._extract_username_from_url(resume.contact_info.linkedin, "linkedin")
            })
        if resume.contact_info.github:
            social_networks.append({
                "network": "GitHub", 
                "username": self._extract_username_from_url(resume.contact_info.github, "github")
            })
        ...
        if resume.projects:
            ...
            for project in resume.projects:
                proj_data = {
                    "name": project.name,
                    "date": project.end_date.strftime("%Y-%m") if project.end_date else "",
...
class ResumeExportService:
    ...
    async def _export_json(self, resume: Resume, options: Dict[str, Any]) -> Dict[str, Any]:
        """Export resume as JSON."""
        try:
            output_filename = options.get("filename") or f"{resume.contact_info.full_name.replace(' ', '_')}_resume.json"
    ...
    async def _export_yaml(self, resume: Resume, options: Dict[str, Any]) -> Dict[str, Any]:
        """Export resume as YAML (RenderCV format)."""
        try:
            output_filename = options.get("filename") or f"{resume.contact_info.full_name.replace(' ', '_')}_resume.yaml"
            output_path = self.pdf_service.output_dir / output_filename
    ...
    async def _export_text(self, resume: Resume, options: Dict[str, Any]) -> Dict[str, Any]:
        """Export resume as plain text."""
        try:
            output_filename = options.get("filename") or f"{resume.contact_info.full_name.replace(' ', '_')}_resume.txt"
            output_path = self.pdf_service.output_dir / output_filename
    ...
    def _generate_text_resume(self, resume: Resume) -> str:
        ...
        if resume.work_experience:
            ...
            for exp in resume.work_experience:
                ...
                date_range += f"{exp.end_date.strftime('%Y-%m') if exp.end_date else 'Present'}"
