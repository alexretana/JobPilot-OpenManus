...
from app.data.resume_models import Resume, ResumeType, ResumeStatus
from app.data.models import JobListingDB, UserProfileDB
from app.services.resume_generation_service import ResumeGenerationService, ResumeContentBuilder
from app.services.pdf_generation_service import PDFGenerationService, ResumeExportService
...
class ResumeOrchestratorService:
    ...
    async def generate_complete_resume(
        ...
        try:
            logger.info(f"Starting complete resume generation for user {request.user_id}")
            ...
            for export_format, task in export_tasks:
                try:
                    ...
                    if export_result.get("success"):
                        ...
                        result.warnings.append(f"Export to {export_format} failed: {export_result.get('error', 'Unknown error')}")
                        
                except Exception as e:
                    result.warnings.append(f"Export to {export_format} failed: {str(e)}")
            
            # Step 4: Update skill bank if resume was created/optimized
            if request.generation_type in ["create", "optimize"]:
                try:
                    await self.resume_repository.update_skill_bank_from_resume(request.user_id, resume)
            ...
            logger.info(f"Complete resume generation finished in {result.processing_time:.2f}s")
            return result
    ...
    async def _create_new_resume(
        ...
        try:
            # Get user profile
            user_profile = self.resume_repository.session.query(UserProfileDB).filter(
    ...
    async def _optimize_existing_resume(
        ...
        try:
            ...
            job_listing = None
            if request.job_id:
                job_listing = self.resume_repository.session.query(JobListingDB).filter(
                    JobListingDB.id == request.job_id
                ).first()
            ...
            if job_listing:
                optimization_result = await self.generation_service.optimize_resume_for_job(
                ...
                if request.job_description:
                    ...
                    raise ValueError("Either job_id or job_description required for optimization")
        ...
        try:
            ...
            if request.job_description:
                ...
                common_keywords = [
                    ...
                    "leadership", "agile", "scrum", "ci/cd", "api", "microservices"
                ]
                target_keywords = [
            ...
            for exp in enhanced_resume.work_experience:
                try:
                    enhanced_exp = await self.generation_service.enhance_work_experience(
                        experience=exp,
                    ...
                    logger.warning(f"Failed to enhance experience at {exp.company}: {e}")
                    enhanced_experiences.append(exp)  # Use original if enhancement fails
    ...
    def _analyze_resume_content(self, resume: Resume) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "summary_word_count": len(resume.summary.split()) if resume.summary else 0,
    ...
    async def batch_generate_resumes(
        self,
        requests: List[ResumeGenerationRequest],
        ...
        async def generate_single(request: ResumeGenerationRequest) -> ResumeGenerationResult:
        ...
        logger.info(f"Batch generation completed: {len(final_results)} resumes processed")
        ...
        try:
            ...
            return {
                ...
                "optimization_levels": [
                    ...
                    {"value": "moderate", "description": "Balanced content and keyword optimization"},
                    {"value": "aggressive", "description": "Comprehensive content rewriting"}
                ],
                "generation_types": [
                    {"value": "create", "description": "Create new resume from profile"},
                    {"value": "optimize", "description": "Optimize existing resume for job"},
            ...
            return {
                ...
                "pdf_templates": [{"name": "moderncv", "description": "Default template"}],
                "export_formats": ["pdf"]
            }
