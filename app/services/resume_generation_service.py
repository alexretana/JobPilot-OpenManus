...
from app.data.resume_models import (
    Resume, WorkExperience, Education, Skill, Project, Certification,
    ContactInfo, ResumeType, ResumeStatus, SkillLevel, ExperienceType
)
...
class ResumeGenerationService:
    ...
    async def generate_professional_summary(
        ...
        try:
            ...
            try:
                ...
                logger.info(f"Generated {len(result.get('summaries', []))} summary variations")
                return result
            except json.JSONDecodeError as e:
                logger.error(f"Failed to parse LLM response as JSON: {e}")
                # Return a fallback summary
                return self._generate_fallback_summary(target_role, key_skills, experience_level)
    ...
    async def generate_achievement_bullets(
        ...
        try:
            ...
            try:
                result = json.loads(response)
                logger.info(f"Generated {len(result.get('achievements', []))} achievement bullets")
                return result
            except json.JSONDecodeError as e:
                logger.error(f"Failed to parse LLM response as JSON: {e}")
    ...
    async def optimize_resume_for_job(
        ...
        try:
            ...
            preferred_skills = []
            if hasattr(job_listing, 'skills_preferred') and job_listing.skills_preferred:
            ...
            try:
                result = json.loads(response)
                logger.info(f"Optimized resume for {job_listing.title} at {job_listing.company}")
                return result
            except json.JSONDecodeError as e:
    ...
    def create_optimized_resume(
        ...
        try:
            ...
            optimized_resume = Resume(
                ...
                summary=optimization_result.get('optimized_summary', base_resume.summary),
            ...
            logger.info(f"Created optimized resume with {len(optimized_resume.work_experience)} experiences")
    ...
    def _generate_fallback_summary(self, target_role: str, key_skills: List[str], experience_level: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "customization_notes": ["Fallback summary - recommend manual customization"],
            "ats_optimization": {
    ...
    def _generate_fallback_achievements(self, position_title: str, technologies: List[str]) -> Dict[str, Any]:
...
class ResumeContentBuilder:
    ...
    async def build_from_profile(
        ...
        try:
            ...
            summary_result = await self.generation_service.generate_professional_summary(
            ...
            logger.info(f"Built base resume for {user_profile.first_name} {user_profile.last_name}")
            return resume
