...
interface Lead {
  ...
  status: 'new' | 'contacted' | 'responded' | 'meeting_scheduled' | 'follow_up_needed' | 'closed_won' | 'closed_lost';
  source?: string;
  notes?: string;
...
const LeadsManager = () => {
  ...
  const getStatusBadgeClass = (status: Lead['status']) => {
    ...
    switch (status) {
      case 'new': return `${baseClass} badge-neutral`;
      case 'contacted': return `${baseClass} badge-info`;
      case 'responded': return `${baseClass} badge-success`;
      case 'meeting_scheduled': return `${baseClass} badge-warning`;
      case 'follow_up_needed': return `${baseClass} badge-accent`;
      case 'closed_won': return `${baseClass} badge-success`;
      case 'closed_lost': return `${baseClass} badge-error`;
  ...
  const getTypeBadgeClass = (type: Lead['lead_type']) => {
    ...
    switch (type) {
      case 'recruiter': return `${baseClass} badge-primary`;
      case 'hiring_manager': return `${baseClass} badge-secondary`;
      case 'employee': return `${baseClass} badge-accent`;
      case 'referral': return `${baseClass} badge-success`;
      case 'networking': return `${baseClass} badge-info`;
        ...
        <div class="stats stats-horizontal shadow">
          ...
          <div class="stat">
            ...
            <div class="stat-value text-lg">
              {stats() ? Math.round(((stats()!.status_breakdown.responded || 0) / Math.max(stats()!.total_leads, 1)) * 100) : 0}%
            </div>
          </div>
        </div>
        ...
        <div class="grid gap-4">
          ...
          >
            ...
            >
              <For each={leads()}>
                {(lead) => (
                  <div class={`card bg-base-100 shadow-xl ${isFollowUpOverdue(lead) ? 'ring-2 ring-warning' : ''}`}>
                    <div class="card-body">
                      <div class="flex flex-col lg:flex-row justify-between items-start gap-4">
                        <div class="flex-1">
                          ...
                          <div class="flex flex-wrap gap-4 mt-3 text-sm">
                            ...
                            {lead.linkedin_url && (
                              <a href={lead.linkedin_url} target="_blank" rel="noopener noreferrer" class="link link-info">
                                💼 LinkedIn
                              </a>
                            )}
                          </div>
                          ...
                          {lead.follow_up_date && (
                            <div class="mt-2">
                              <span class="text-sm text-base-content/40">
                                ...
                                {isFollowUpOverdue(lead) && <span class="text-warning ml-1">⚠️</span>}
                              </span>
                            </div>
                          )}
                          ...
                          {lead.notes && (
                            <div class="mt-3">
                              ...
                              <p class="text-sm text-base-content/60 whitespace-pre-wrap">{lead.notes}</p>
                            </div>
                          )}
                        </div>
