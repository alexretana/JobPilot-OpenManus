...
def main():
    ...
    parser.add_argument("--user-profiles", action="store_true", help="Run user profiles tests only")
    parser.add_argument("--syntax-check", action="store_true", help="Run syntax check only")
    ...
    # Syntax checking
    if args.syntax_check or args.quick or args.full or not any([args.quick, args.full, args.user_profiles]):
        ...
        for filepath in syntax_files:
            tests_run += 1
            if run_command(f"python -m py_compile {filepath}", f"Syntax check: {filepath}"):
                tests_passed += 1
            else:
                tests_failed += 1
    ...
    if args.quick or args.full or args.user_profiles or not any([args.syntax_check]):
        ...
        import_tests = [
            ("from app.data.models import UserProfile, JobType, RemoteType", "Core models"),
            ("from app.api.user_profiles import router, UserProfileCreate", "API components"),
            ("from app.data.database import get_user_repository, initialize_database", "Database components")
        ]
        
        for import_cmd, description in import_tests:
            tests_run += 1
            if run_command(f'python -c "{import_cmd}; print(\'SUCCESS: {description} imported successfully\')"', 
    ...
    if args.user_profiles or args.full:
        ...
        tests_run += 1
        if run_command("python test_user_profiles.py", "User Profiles comprehensive tests"):
            tests_passed += 1
        else:
            tests_failed += 1
...
'''
    ...
    if args.full and Path("test_core_components.py").exists():
        ...
        tests_run += 1
        if run_command("python test_core_components.py", "Core component tests", continue_on_error=True):
            tests_passed += 1
        else:
            tests_failed += 1
            print("⚠️ Core component tests failed - this may be expected if dependencies are missing")

    # Backend API tests (if available)
    if args.full and Path("tests/backend").exists():
        print_step("Backend API Tests", "🌐")
        tests_run += 1
        if run_command("pytest tests/backend/ -v --tb=short --disable-warnings", "Backend API tests", continue_on_error=True):
            tests_passed += 1
        else:
            tests_failed += 1
