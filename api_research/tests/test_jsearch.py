...
async def comprehensive_jsearch_test(api_key: str):
    ...
    async with JSearchClient(api_key) as client:
        try:
            ...
            print(f"   Found {len(jobs_filtered)} full-time data scientist jobs posted this week")
            ...
            if all_jobs:
                ...
                for field, stats in fields_analysis.items():
                    print(f"  {field}: {stats['filled']}/{stats['total']} ({stats['percentage']:.1f}%) complete")
                ...
                for i, job in enumerate(all_jobs[:3], 1):
                    ...
                    print(f"  Description (first 200 chars): {job.description[:200] if job.description else 'N/A'}...")
            ...
            print(f"Total requests used: {rate_limits.requests_used}/{rate_limits.requests_per_month}")
            print(f"Requests remaining: {rate_limits.requests_remaining}")
            print(f"Efficiency: {len(all_jobs) / stats['requests_made']:.1f} jobs per API call")
...
def analyze_field_completeness(jobs):
    ...
    fields = [
        ...
        'employment_type', 'date_posted', 'apply_url', 'company_logo',
        'job_requirements', 'benefits', 'source_site'
    ]
    ...
    for field in fields:
        filled_count = sum(1 for job in jobs if getattr(job, field) not in [None, '', []])
        analysis[field] = {
            'total': total_jobs,
            'filled': filled_count,
