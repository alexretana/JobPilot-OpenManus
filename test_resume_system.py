...
from app.data.resume_models import (
    Resume, ResumeDB, ContactInfo, WorkExperience, Education, Skill, Project, 
    Certification, ResumeStatus, ResumeType, SkillLevel, ExperienceType,
    calculate_resume_completeness, generate_ats_score, extract_resume_keywords
)
...
def create_comprehensive_resume(user_id: str) -> Resume:
    ...
    skills = [
        Skill(name="Python", level=SkillLevel.ADVANCED, category="Programming Languages", years_experience=6, is_featured=True),
        Skill(name="JavaScript", level=SkillLevel.ADVANCED, category="Programming Languages", years_experience=5, is_featured=True),
        Skill(name="React", level=SkillLevel.ADVANCED, category="Frontend Frameworks", years_experience=4, is_featured=True),
        Skill(name="Django", level=SkillLevel.ADVANCED, category="Backend Frameworks", years_experience=5, is_featured=True),
        Skill(name="PostgreSQL", level=SkillLevel.INTERMEDIATE, category="Databases", years_experience=4, is_featured=False),
        Skill(name="AWS", level=SkillLevel.INTERMEDIATE, category="Cloud Platforms", years_experience=3, is_featured=False),
        Skill(name="Docker", level=SkillLevel.INTERMEDIATE, category="DevOps", years_experience=3, is_featured=False),
        Skill(name="Kubernetes", level=SkillLevel.BEGINNER, category="DevOps", years_experience=1, is_featured=False)
    ]
...
async def test_resume_crud_operations():
    ...
    try:
        ...
        resume = await repo.create_resume_from_user_profile(user_id, "Software Engineer Resume")
        
        if resume:
            logger.info(f"✅ Resume created successfully: {resume.id}")
            logger.info(f"   Title: {resume.title}")
            logger.info(f"   Contact: {resume.contact_info.full_name} ({resume.contact_info.email})")
            logger.info(f"   Skills: {len(resume.skills)} skills")
        else:
            # If user profile doesn't exist, create comprehensive resume directly
            logger.info("⚠️  User profile not found, creating comprehensive resume directly...")
        ...
        if retrieved_resume:
            ...
            logger.info(f"   Work Experience: {len(retrieved_resume.work_experience)} positions")
        ...
        if updated_resume:
            logger.info(f"✅ Resume updated successfully (version {updated_resume.version})")
...
async def test_ats_scoring(resume: Resume):
    ...
    try:
        ...
        logger.info(f"   Missing Keywords: {len(targeted_score.missing_keywords)} identified")
        if targeted_score.missing_keywords:
            logger.info(f"   Sample Missing: {', '.join(targeted_score.missing_keywords[:5])}")
...
async def test_skill_bank():
    ...
    try:
        ...
        if skill_bank:
            ...
            logger.info(f"   Technical Keywords: {len(skill_bank['technical_keywords'])}")
...
async def test_resume_analytics(resume: Resume):
    ...
    try:
        ...
        logger.info(f"   Total Experience: {sum(exp.end_date.year - exp.start_date.year if exp.end_date else datetime.now().year - exp.start_date.year for exp in resume.work_experience)} years")
        logger.info(f"   Education Levels: {len(resume.education)}")
        logger.info(f"   Technical Skills: {len([s for s in resume.skills if s.is_featured])}")
