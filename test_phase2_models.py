...
def test_new_pydantic_models():
    ...
    try:
        from app.data.models import (
            JobSource, JobSourceListing, JobEmbedding, JobDeduplication,
            JobListing, VerificationStatus, CompanySizeCategory, SeniorityLevel
        )
...
def test_sqlalchemy_models():
    ...
    try:
        from app.data.models import (
            Base, JobSourceDB, JobSourceListingDB, JobEmbeddingDB, 
            JobDeduplicationDB, JobListingDB, create_database_engine, create_tables
        )
        ...
        expected_new_tables = [
            "job_sources", "job_source_listings", "job_embeddings", "job_duplications"
        ]
        
        existing_tables = [
            "job_listings", "user_profiles", "applications", "saved_jobs", 
            "companies", "timeline_events"
        ]
        ...
        job_listings_columns = [col['name'] for col in inspector.get_columns('job_listings')]
        new_columns = [
            'canonical_id', 'source_count', 'data_quality_score', 'scraped_at',
            'last_verified', 'verification_status', 'company_size_category',
            'seniority_level', 'tech_stack', 'benefits_parsed'
        ]
...
def test_model_conversions():
    ...
    try:
        from app.data.models import (
            JobSource, JobSourceDB, pydantic_to_sqlalchemy, sqlalchemy_to_pydantic
        )
...
def test_enum_functionality():
    ...
    try:
        ...
        print(f"   ✅ VerificationStatus has {len(statuses)} values: {[s.value for s in statuses]}")
        ...
        print(f"   ✅ CompanySizeCategory has {len(sizes)} values: {[s.value for s in sizes]}")
        ...
        print(f"   ✅ SeniorityLevel has {len(levels)} values: {[s.value for s in levels]}")
