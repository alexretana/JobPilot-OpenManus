...
from app.data.resume_models import (
    Resume, ContactInfo, WorkExperience, Education, Skill, Project, 
    Certification, ResumeStatus, ResumeType, SkillLevel, ExperienceType
)
from app.repositories.resume_repository import ResumeRepository
from app.services.resume_orchestrator_service import (
    ResumeOrchestratorService, ResumeGenerationRequest,
...
class ResumeGenerationTester:
    ...
    def create_comprehensive_resume(self) -> Resume:
        ...
        work_experience = [
            WorkExperience(
                ...
                skills_used=["Python", "AWS", "Kubernetes", "Docker", "PostgreSQL", "Redis"]
            ),
            WorkExperience(
                company="Microsoft",
                ...
                skills_used=["C#", ".NET Core", "Azure", "SQL Server", "React", "TypeScript"]
            )
        ]
        ...
        education = [
            Education(
                ...
                relevant_coursework=[
                    "Distributed Systems", "Machine Learning", "Database Systems",
        ...
        skills = [
            Skill(name="Python", level=SkillLevel.EXPERT, category="Programming Languages", years_experience=6, is_featured=True),
            Skill(name="Java", level=SkillLevel.ADVANCED, category="Programming Languages", years_experience=5, is_featured=True),
            Skill(name="JavaScript", level=SkillLevel.ADVANCED, category="Programming Languages", years_experience=4, is_featured=True),
            Skill(name="AWS", level=SkillLevel.ADVANCED, category="Cloud Platforms", years_experience=4, is_featured=True),
            Skill(name="Kubernetes", level=SkillLevel.INTERMEDIATE, category="DevOps", years_experience=3, is_featured=False),
            Skill(name="Docker", level=SkillLevel.ADVANCED, category="DevOps", years_experience=4, is_featured=False),
            Skill(name="PostgreSQL", level=SkillLevel.ADVANCED, category="Databases", years_experience=5, is_featured=False),
            Skill(name="React", level=SkillLevel.INTERMEDIATE, category="Frontend Frameworks", years_experience=3, is_featured=False),
            Skill(name="Machine Learning", level=SkillLevel.INTERMEDIATE, category="AI/ML", years_experience=2, is_featured=False)
        ]
    ...
    async def test_llm_service(self):
        ...
        try:
            ...
            if content and len(content) > 10:
                self.log_test_result("LLM Content Generation", True, f"Generated {len(content)} characters")
            else:
                self.log_test_result("LLM Content Generation", False, "No meaningful content generated")
            
            # Test provider info
            provider_info = llm_service.get_provider_info()
            if provider_info.get("provider") == "mock":
                self.log_test_result("LLM Provider Info", True, f"Provider: {provider_info['provider']}")
            else:
                self.log_test_result("LLM Provider Info", False, "Provider info incorrect")
    ...
    async def test_pdf_generation_service(self):
        ...
        try:
            ...
            if templates and len(templates) > 0:
                self.log_test_result("PDF Templates", True, f"Found {len(templates)} templates")
            else:
            ...
            if rendercv_data and "cv" in rendercv_data:
                self.log_test_result("RenderCV Conversion", True, "Successfully converted to RenderCV format")
            else:
                self.log_test_result("RenderCV Conversion", False, "Failed to convert to RenderCV format")
        ...
        try:
            with self.db_manager.get_session() as session:
                ...
                if summary_result and "summaries" in summary_result:
                    self.log_test_result("Professional Summary Generation", True, 
                                       f"Generated {len(summary_result['summaries'])} summary variations")
                else:
                    self.log_test_result("Professional Summary Generation", False, "No summaries generated")
                ...
                if achievement_result and "achievements" in achievement_result:
                    self.log_test_result("Achievement Generation", True,
                                       f"Generated {len(achievement_result['achievements'])} achievements")
                else:
                    self.log_test_result("Achievement Generation", False, "No achievements generated")
        ...
        try:
            with self.db_manager.get_session() as session:
                ...
                if templates and "pdf_templates" in templates:
                    self.log_test_result("Orchestrator Templates", True, 
                                       f"Retrieved templates and options")
                else:
                    self.log_test_result("Orchestrator Templates", False, "Failed to get templates")
                ...
                try:
                    result = await orchestrator.generate_complete_resume(creation_request)
                    if result.errors and "User profile not found" in str(result.errors):
                        self.log_test_result("Orchestrator Structure", True, 
                                           "Orchestrator properly handles missing user profile", warning=True)
                    else:
                        self.log_test_result("Orchestrator Create Resume", result.success,
                                           f"Resume creation: {result.errors if not result.success else 'Success'}")
                except Exception as e:
                    if "User profile not found" in str(e):
                        self.log_test_result("Orchestrator Structure", True,
                                           "Orchestrator properly validates user profile", warning=True)
                    else:
                        raise
    ...
    async def test_end_to_end_workflow(self):
        ...
        try:
            with self.db_manager.get_session() as session:
                ...
                self.log_test_result("Base Resume Creation", True, f"Created resume: {resume_db.id}")
                ...
                result = await orchestrator.generate_complete_resume(enhancement_request)
                
                if result.success:
                    self.log_test_result("E2E Enhancement", True, 
                                       f"Enhanced resume created: {result.resume_id}")
                    self.log_test_result("E2E Export", len(result.generated_files) >= 2,
                                       f"Generated {len(result.generated_files)} export files")
                    
                    # Check processing metadata
                    if result.generation_metadata:
                        self.log_test_result("E2E Metadata", True,
                                           f"Processing time: {result.processing_time:.2f}s")
                    
                else:
                    self.log_test_result("E2E Enhancement", False, f"Errors: {result.errors}")
    ...
    async def test_export_functionality(self):
        ...
        try:
            ...
            if json_result.get("success"):
                self.log_test_result("JSON Export", True, f"Exported to {json_result['filename']}")
            else:
                self.log_test_result("JSON Export", False, f"Error: {json_result.get('error')}")
            ...
            if yaml_result.get("success"):
                self.log_test_result("YAML Export", True, f"Exported to {yaml_result['filename']}")
            else:
                self.log_test_result("YAML Export", False, f"Error: {yaml_result.get('error')}")
            ...
            if txt_result.get("success"):
                self.log_test_result("TXT Export", True, f"Exported to {txt_result['filename']}")
            else:
                self.log_test_result("TXT Export", False, f"Error: {txt_result.get('error')}")
    ...
    async def test_performance_benchmarks(self):
        ...
        try:
            with self.db_manager.get_session() as session:
                ...
                if result.success and processing_time < 10.0:  # Should complete in under 10 seconds
                    self.log_test_result("Performance Single", True, 
                                       f"Generated in {processing_time:.2f}s (target: <10s)")
                else:
                    self.log_test_result("Performance Single", False,
                ...
                batch_results = await orchestrator.batch_generate_resumes(batch_requests, max_concurrent=2)
                batch_time = (datetime.now() - batch_start).total_seconds()
                
                successful_batch = sum(1 for r in batch_results if r.success)
                
                if successful_batch >= 2 and batch_time < 30.0:  # Should complete batch in under 30s
                    self.log_test_result("Performance Batch", True,
                                       f"Generated {successful_batch}/3 in {batch_time:.2f}s")
                else:
                    self.log_test_result("Performance Batch", False,
    ...
    def print_summary(self):
        ...
        if self.test_results["failed"] == 0:
            print("🎉 ALL TESTS PASSED! Resume generation pipeline is working correctly.")
        else:
            print(f"❌ {self.test_results['failed']} tests failed. Please review the issues above.")
