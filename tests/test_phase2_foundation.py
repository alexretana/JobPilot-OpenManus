...
def test_database_schema():
    ...
    try:
        ...
        expected_tables = [
            "job_sources", "job_source_listings", "job_embeddings", 
            "job_duplications", "job_listings", "user_profiles", 
            "applications", "saved_jobs", "companies", "timeline_events"
        ]
...
async def test_vector_store():
    ...
    try:
        ...
        hybrid_matches = await vector_store.hybrid_search(
            "Python developer with database experience",
            limit=2
        )
...
async def test_integration():
    ...
    try:
        ...
        embedding = await vector_store.store_job_embedding(test_job)
        print("   ✅ Embedding created and stored")
        
        # Test search
        matches = await vector_store.find_similar_jobs("machine learning python", limit=1)
        if matches:
