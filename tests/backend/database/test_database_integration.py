...
try:
    from app.data.database import get_database_manager, DatabaseManager
    from app.data.models import (
        JobListingDB, UserProfileDB, ApplicationDB, SavedJobDB, CompanyDB,
        TimelineEventDB, JobSourceDB, JobSourceListingDB, JobEmbeddingDB,
        JobDeduplicationDB, VerificationStatus, CompanySizeCategory, 
        SeniorityLevel, TimelineEventType
    )
...
class TestDatabaseBasics:
    ...
    def test_table_creation(self, temp_database):
        ...
        expected_tables = [
            'job_listings', 'user_profiles', 'applications', 'saved_jobs',
            'companies', 'timeline_events', 'job_sources', 'job_source_listings',
            'job_embeddings', 'job_duplications'
        ]
...
class TestJobListingCRUD:
    ...
    def test_read_job_listing(self, db_session, sample_job_data):
        ...
        # Read job
        retrieved_job = db_session.query(JobListingDB).filter(JobListingDB.id == job_id).first()
    ...
    def test_delete_job_listing(self, db_session, sample_job_data):
        ...
        # Verify deletion
        deleted_job = db_session.query(JobListingDB).filter(JobListingDB.id == job_id).first()
...
class TestModelRelationships:
    ...
    def test_job_company_relationship(self, db_session, sample_job_data, sample_company_data):
    ...
    def test_user_application_relationship(self, db_session, sample_user_data, sample_job_data):
...
class TestTransactions:
    ...
    def test_transaction_commit(self, db_session, sample_job_data):
        ...
        saved_job = db_session.query(JobListingDB).filter(JobListingDB.id == job.id).first()
    ...
    def test_transaction_rollback(self, db_session, sample_job_data):
        ...
        try:
            ...
            jobs = db_session.query(JobListingDB).filter(JobListingDB.id == job.id).all()
            assert len(jobs) == 0
    ...
    def test_batch_operations(self, db_session):
        ...
        saved_jobs = db_session.query(JobListingDB).filter(
            JobListingDB.company.like('Company %')
        ).all()
...
class TestDatabasePerformance:
    ...
    def test_query_performance(self, db_session):
        ...
        # Query by company
        company_jobs = db_session.query(JobListingDB).filter(
            JobListingDB.company == 'Company 1'
        ).all()
        
        # Query by salary range
        salary_jobs = db_session.query(JobListingDB).filter(
            JobListingDB.salary_min >= 60000,
            JobListingDB.salary_max <= 120000
        ).all()
        
        # Query by location
        location_jobs = db_session.query(JobListingDB).filter(
            JobListingDB.location.like('City%')
        ).all()
        ...
        print(f"Found {len(company_jobs)} company jobs, {len(salary_jobs)} salary jobs, {len(location_jobs)} location jobs")
