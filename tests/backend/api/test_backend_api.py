...
class BackendAPITester:
        ...
        try:
            ...
            for source_data in job_sources:
                async with self.session.post(
                    f"{self.base_url}/api/job-sources",
                    json=source_data
                ) as response:
                ...
                if response.status == 200:
                    ...
                    for created_source in created_sources:
                        if created_source['name'] not in source_names:
                            print(f"   ❌ Created source {created_source['name']} not found in list")
                            return False
                else:
            ...
            if created_sources:
                ...
                async with self.session.put(
                    ...
                    if response.status == 200:
                        ...
                        print(f"   ✅ Updated job source: {updated_source['display_name']}")
            ...
            enhanced_jobs = [
                {
                    ...
                    "salary_min": 150000,
                    "salary_max": 200000,
                    "skills_required": ["Python", "Machine Learning", "TensorFlow", "AWS"],
                    "tech_stack": ["Python", "TensorFlow", "Docker", "Kubernetes", "AWS"],
                    # New Phase 2 fields
            ...
            for job_data in enhanced_jobs:
                async with self.session.post(
                    ...
                    if response.status == 200:
                        ...
                        print(f"   ✅ Created enhanced job: {job['title']} at {job['company']}")
                    else:
                ...
                if response.status == 200:
                    ...
                    for job in jobs:
                        if job['id'] in [j['id'] for j in created_jobs]:
                            required_fields = [
                                'verification_status', 'company_size_category', 
                                'seniority_level', 'data_quality_score', 'tech_stack'
                            ]
                            for field in required_fields:
                                if field not in job:
                                    print(f"   ❌ Missing enhanced field '{field}' in job {job['id']}")
                                    return False
            ...
            for filter_field, filter_value in filter_tests:
                async with self.session.get(
                    ...
                    if response.status == 200:
                        ...
                        print(f"   ✅ Filter by {filter_field}={filter_value}: {len(filtered_jobs)} results")
                    else:
                        print(f"   ❌ Failed to filter by {filter_field}: {response.status}")
                        return False
    ...
    async def test_job_source_listings_api(self) -> bool:
        ...
        try:
            if not self.test_data.get('job_sources') or not self.test_data.get('enhanced_jobs'):
            ...
            async with self.session.post(
                f"{self.base_url}/api/job-source-listings",
                json=source_listing_data
            ) as response:
                if response.status == 201:
                    source_listing = await response.json()
                    print(f"   ✅ Created job source listing: {source_listing['source_job_id']}")
                    self.test_data['source_listing'] = source_listing
                else:
                    error_text = await response.text()
                    print(f"   ❌ Failed to create job source listing: {response.status} - {error_text}")
                    return False
        ...
        try:
            ...
            for query in search_queries:
                async with self.session.get(
                    ...
                    if response.status == 200:
                        ...
                        print(f"   ❌ Semantic search failed for '{query}': {response.status}")
                        return False
            ...
            async with self.session.get(
                ...
                if response.status == 200:
                    ...
                    print(f"   ✅ Filtered semantic search: {len(filtered_results)} results")
                else:
        ...
        try:
            ...
            async with self.session.post(
                ...
                if response.status == 201:
                    ...
                    error_text = await response.text()
                    print(f"   ❌ Failed to create embedding: {response.status} - {error_text}")
                    return False
            ...
            async with self.session.get(
                ...
                if response.status == 200:
                    ...
                    print(f"   ✅ Embeddings stats: {stats.get('total_embeddings', 0)} total")
                else:
        ...
        try:
            ...
            async with self.session.post(
                ...
                if response.status == 200:
                    ...
                    print(f"   ✅ Deduplication check: {dedup_result.get('confidence_score', 0):.2f} confidence")
                    ...
                    error_text = await response.text()
                    print(f"   ❌ Deduplication check failed: {response.status} - {error_text}")
                    return False
            
            # Test batch deduplication
            async with self.session.post(
                f"{self.base_url}/api/jobs/deduplicate-batch",
                json={"job_ids": [job['id'] for job in self.test_data['enhanced_jobs']]}
            ) as response:
                if response.status == 200:
                    batch_results = await response.json()
                    print(f"   ✅ Batch deduplication: found {len(batch_results.get('duplicates', []))} potential duplicates")
                else:
        ...
        try:
            ...
            async with self.session.post(
                ...
                if response.status == 200:
                    ...
                    error_text = await response.text()
                    print(f"   ❌ Failed to create timeline event: {response.status} - {error_text}")
                    return False
    ...
    async def test_statistics_and_analytics(self) -> bool:
        ...
        try:
            ...
            async with self.session.get(f"{self.base_url}/api/stats") as response:
                if response.status == 200:
                    stats = await response.json()
                    print(f"   ✅ General stats: {stats.get('total_jobs', 0)} total jobs")
                else:
                    print(f"   ❌ Failed to get general stats: {response.status}")
                    return False
            
            # Test Phase 2 enhanced stats
            async with self.session.get(f"{self.base_url}/api/stats/enhanced") as response:
                if response.status == 200:
                    enhanced_stats = await response.json()
                    print(f"   ✅ Enhanced stats retrieved with quality and source metrics")
                elif response.status == 404:
            ...
            async with self.session.get(f"{self.base_url}/api/stats/sources") as response:
                if response.status == 200:
                    source_stats = await response.json()
    ...
    async def cleanup_test_data(self) -> bool:
        ...
        try:
            ...
            for job in self.test_data.get('enhanced_jobs', []):
                async with self.session.delete(f"{self.base_url}/api/leads/{job['id']}") as response:
                    if response.status in [200, 204, 404]:  # 404 is OK if already deleted
                        cleanup_count += 1
                    else:
                        print(f"   ⚠️ Failed to delete job {job['id']}: {response.status}")
            
            # Delete job sources (if endpoint exists)
            for source in self.test_data.get('job_sources', []):
                async with self.session.delete(f"{self.base_url}/api/job-sources/{source['id']}") as response:
            ...
            if 'timeline_event' in self.test_data:
                ...
                async with self.session.delete(f"{self.base_url}/api/timeline/event/{event['id']}") as response:
                    if response.status in [200, 204, 404]:
                        cleanup_count += 1
    ...
    async def run_all_tests(self) -> bool:
        ...
        if passed_tests == len(results):
            print("🎉 All backend API tests passed! Phase 2 functionality is working correctly.")
            return True
        elif passed_tests >= len(results) * 0.75:  # 75% pass rate
            print("⚠️ Most tests passed. Some Phase 2 endpoints may not be fully implemented yet.")
            return True
        else:
...
async def main():
    ...
    parser.add_argument("--single-test", choices=[
        "health", "sources", "jobs", "search", "embeddings", 
        "dedup", "timeline", "stats"
