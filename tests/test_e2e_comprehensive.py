...
class ComprehensiveE2ETestSuite:
    ...
    def _record_result(self, name: str, passed: bool, duration: float, details: str = "", error: str = None):
        """Record test result."""
    ...
    async def _timed_test(self, test_name: str, test_func):
        ...
        try:
            ...
            if result:
                ...
                self._record_result(test_name, True, duration, "Test completed successfully")
            else:
                print(f"   ❌ FAILED ({duration:.2f}s)")
                self._record_result(test_name, False, duration, "Test assertions failed")
    ...
    async def test_job_crud_operations(self) -> bool:
        ...
        try:
            ...
            for job_data in job_data_list:
                async with self.session.post(f"{self.base_url}/api/leads", json=job_data) as response:
                    if response.status == 200:
            ...
            if test_jobs:
                ...
                async with self.session.put(
                    f"{self.base_url}/api/leads/{job_to_update['id']}", 
                    json=update_data
                ) as response:
                    if response.status == 200:
                        updated_job = await response.json()
                        print(f"   ✅ Updated job salary: ${updated_job['salary_max']:,}")
                    else:
                        return False
        ...
        try:
            ...
            try:
                ...
                if rapidapi_key and rapidapi_key != 'test_key':
                    ...
                    async with self.session.get(f"{self.base_url}/api/etl/status") as response:
                        if response.status == 200:
                ...
                async with self.session.post(f"{self.base_url}/api/leads", json=test_job) as response:
                    if response.status == 200:
    ...
    async def test_full_user_journey(self) -> bool:
        ...
        try:
            ...
            async with self.session.get(f"{self.base_url}/api/leads/{created_job['id']}") as response:
                if response.status == 200:
            ...
            if await self.page.query_selector('body'):
                try:
                    ...
                    if test_job['title'] in page_content or test_job['company'] in page_content:
                        print("   ✅ UI: Job data appears in interface")
                    else:
                        print("   ℹ️ UI: Job data not visible (may require specific view)")
    ...
    async def test_data_persistence(self) -> bool:
        ...
        try:
            ...
            async with self.session.get(f"{self.base_url}/api/leads/{job['id']}") as response:
                if response.status == 200:
                    ...
                    if persistent_job['title'] == new_title:
                        ...
                        print(f"   ❌ Data not persisted. Expected: '{new_title}', Got: '{persistent_job['title']}'")
                        return False
    ...
    def _generate_summary(self) -> Dict[str, Any]:
        ...
        summary = {
            ...
            'recommendation': self._get_recommendation(len(passed_tests), len(self.results))
        }
    ...
    def _get_recommendation(self, passed: int, total: int) -> str:
        ...
        if pass_rate >= 95:
            ...
            return "🔧 Significant issues found. System needs attention before proceeding."


# ================== Main Runners ==================

async def run_e2e_tests_with_servers(include_frontend: bool = False, rapidapi_key: str = None):
...
async def run_e2e_tests_manual_server():
    ...
    async with ComprehensiveE2ETestSuite() as test_suite:
        ...
        try:
            async with test_suite.session.get(f"{test_suite.base_url}/api/health", timeout=5) as response:
                if response.status != 200:
                    print("❌ Server health check failed. Please ensure server is running on localhost:8080")
...
async def main():
    ...
    parser = argparse.ArgumentParser(description="JobPilot Comprehensive E2E Tests with Playwright")
    parser.add_argument("--rapidapi-key", help="RapidAPI key for ETL testing")
    parser.add_argument("--include-frontend", action="store_true", help="Start frontend server")
    parser.add_argument("--manual-server", action="store_true", help="Use manually started server")
    parser.add_argument("--headless", default=True, type=bool, help="Run browser tests in headless mode")
