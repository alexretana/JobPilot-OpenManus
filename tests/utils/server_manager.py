...
class ServerManager:
    ...
    async def start_frontend(self) -> bool:
        ...
        try:
            ...
            print(f"   ⚠️ Frontend server start failed: {e} (continuing without frontend)")
    ...
    async def health_check(self) -> Dict[str, bool]:
        ...
        # Check backend
        try:
            response = requests.get(f"http://localhost:{self.ports['backend']}/api/health", timeout=5)
    ...
    @asynccontextmanager
    async def managed_servers(self, start_frontend: bool = False, env_vars: Dict[str, str] = None):
...
class TestEnvironment:
    ...
    async def full_test_environment(self, include_frontend: bool = False):
        ...
        async with self.server_manager.managed_servers(
            start_frontend=include_frontend,
            env_vars=self.test_config
        ) as servers:
            yield servers
...
async def with_test_servers(test_func, include_frontend: bool = False, env_vars: Dict[str, str] = None):
