...
class ServerManager:
    ...
    def wait_for_server(
        self, port: int, timeout: int = 30, endpoint: str = "/"
    ) -> bool:
    ...
    def start_backend_server(self) -> bool:
        ...
        try:
            ...
            if self.wait_for_server(
                self.backend_port, self.backend_timeout, "/api/health"
            ):
                logger.info(
    ...
    def health_check(self) -> Dict[str, bool]:
        ...
        try:
            response = requests.get(
                f"http://localhost:{self.backend_port}/api/health", timeout=5
            )
...
@contextmanager
def test_servers(
    backend_port: int = 8001, frontend_port: int = 3001, start_frontend: bool = True
):
    ...
    try:
        ...
        if start_frontend and not frontend_started:
            logger.warning(
                "Frontend server failed to start, continuing with backend only"
            )
        ...
        for proc in psutil.process_iter(["pid", "name", "cmdline"]):
            try:
                ...
                if any("web_server.py" in str(cmd) for cmd in cmdline):
                    ...
                    "npm run dev" in " ".join(str(cmd) for cmd in cmdline)
                    for cmd in [cmdline]
                ):
                    logger.info(
                        f"Killing leftover frontend process: {proc.info['pid']}"
                    )
                    proc.terminate()
