...
class JobDataFactory:
    ...
    def create(cls, **overrides) -> Dict[str, Any]:
        ...
        job_data = {
            ...
            "tech_stack": random.sample(
                skills + ["Git", "Linux", "Agile"], k=min(5, len(skills) + 2)
            ),
    ...
    def _get_skill_category(cls, title: str) -> str:
        ...
        if any(
            word in title_lower
            for word in ["data", "scientist", "analyst", "ml", "machine learning"]
        ):
            return "data"
        elif any(
            word in title_lower
            for word in ["frontend", "front-end", "ui", "ux", "react", "vue"]
        ):
            return "frontend"
        elif any(
            word in title_lower
            for word in ["devops", "infrastructure", "sre", "platform"]
        ):
            return "devops"
        elif any(
            word in title_lower
            for word in ["mobile", "ios", "android", "react native", "flutter"]
        ):
            return "mobile"
        else:
            return "backend"
    ...
    def _generate_job_description(
        cls, title: str, company: str, skills: List[str]
    ) -> str:
...
class UserDataFactory:
    ...
    def create(cls, **overrides) -> Dict[str, Any]:
        ...
        user_data = {
            ...
            "remote_preference": random.choice(
                [
                    "Remote only",
                    "Hybrid preferred",
                    "No preference",
            ...
            "job_types": random.sample(
                ["Full-time", "Part-time", "Contract"], k=random.randint(1, 2)
            ),
...
class ApplicationDataFactory:
    ...
    @classmethod
    def create(
        cls, job_id: str = None, user_id: str = None, **overrides
    ) -> Dict[str, Any]:
...
class TimelineEventFactory:
    ...
    def create(cls, **overrides) -> Dict[str, Any]:
        ...
        event_data = {
            ...
            "tags": random.sample(
                ["job-search", "interview", "networking", "skills", "application"],
...
def setup_test_database_data(
    db_session, num_jobs: int = 5, num_users: int = 3
) -> Dict[str, List[Any]]:
    ...
    applications = [
        ApplicationDataFactory.create(
            job_id=random.choice(jobs)["name"], user_id=random.choice(users)["id"]
        )
        for _ in range(random.randint(2, 8))
    ]

    return {"jobs": jobs, "users": users, "applications": applications}
