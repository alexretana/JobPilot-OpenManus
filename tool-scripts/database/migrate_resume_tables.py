...
from app.data.models import Base, create_database_engine, UserProfileDB
from app.data.resume_models import (
    ResumeDB, ResumeTemplateDB, SkillBankDB, ResumeGenerationDB,
...
def seed_default_templates(engine):
    ...
    try:
        ...
        if existing_count > 0:
            logger.info(f"Templates already exist ({existing_count} found). Skipping seeding.")
            session.close()
            return True
...
def create_sample_data(engine):
    ...
    try:
        ...
        existing_resumes = session.query(ResumeDB).filter(ResumeDB.user_id == user.id).count()
        ...
        default_template = session.query(ResumeTemplateDB).filter(
            ...
            work_experience=[
                {
                    ...
                    "skills_used": ["Python", "Django", "PostgreSQL", "Docker", "Kubernetes"]
                }
            ],
            education=[
                {
        ...
        sample_skill_bank = SkillBankDB(
            ...
            technical_keywords=["python", "javascript", "django", "react", "aws", "docker"],
            soft_skills=["leadership", "communication", "problem-solving", "teamwork"],
            industry_keywords=["software development", "web applications", "microservices", "agile"],
            skill_confidence={
                "python": 0.9,
                "javascript": 0.8,
                "django": 0.85,
...
def main():
    ...
    try:
        ...
        existing_tables, missing_tables, existing_resume_tables = check_existing_tables(engine)
        ...
        if len(existing_resume_tables) == 0:  # Only if this is a fresh migration
            ...
            if not success:
                logger.warning("Failed to create sample data, but migration was successful!")
