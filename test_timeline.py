...
from app.data.models import create_database_engine, get_session_factory, TimelineEventType
...
def test_timeline_functionality():
    ...
    try:
        ...
        event5 = timeline_service.log_custom_event(
            ...
            job_id="test-job-1",
            event_data={"research_time": "2 hours", "sources": ["company website", "glassdoor"]},
            is_milestone=False
        )
        ...
        for event in user_timeline:
            logger.info(f"  - {event.event_date.strftime('%Y-%m-%d %H:%M')} | {event.event_type.value} | {event.title}")
        ...
        for event in milestones:
            ...
            logger.info(f"  - {milestone_marker} {event.event_date.strftime('%Y-%m-%d %H:%M')} | {event.title}")
        ...
        for event in upcoming:
            logger.info(f"  - 📅 {event.event_date.strftime('%Y-%m-%d %H:%M')} | {event.title}")
        ...
        updated_event = timeline_service.update_event(
            ...
            event_data={"research_time": "3 hours", "sources": ["company website", "glassdoor", "linkedin", "news articles"]},
            is_milestone=True
        )
        if updated_event:
