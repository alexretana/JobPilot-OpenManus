...
from app.utils.retry import with_retry, retry_database_operation, is_retryable_error, RetryableError
...
def test_retry_with_jitter():
    ...
    for delay in delays:
        assert 0.5 <= delay <= 10.0  # Should be reasonable for exponential backoff with jitter
